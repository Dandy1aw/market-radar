  ...
  return (
    <Card hover>
      ...
      <div className="flex items-baseline gap-2 mb-4">
        <span className="text-2xl font-bold tabular-nums tracking-tight">{formatPrice(data.close)}</span>
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-sm text-[var(--muted)]">
        {data.pct_change_5d !== null && (
          <>
            <span>5日</span>
            <span className={`tabular-nums font-medium ${getPctColor(data.pct_change_5d)}`}>{formatPct(data.pct_change_5d)}</span>
          </>
        )}
        {data.pct_change_20d !== null && (
          <>
            <span>20日</span>
            <span className={`tabular-nums font-medium ${getPctColor(data.pct_change_20d)}`}>{formatPct(data.pct_change_20d)}</span>
          </>
        )}
        {data.pct_from_ma500 !== null && (
          <>
            <span>距MA500</span>
            <span className={`tabular-nums font-medium ${getPctColor(data.pct_from_ma500)}`}>{formatPct(data.pct_from_ma500)}</span>
          </>
        )}
        {data.drawdown_1y !== null && (
          <>
            <span>年内回撤</span>
            <span className="tabular-nums font-medium text-red-400">{formatPct(data.drawdown_1y)}</span>
          </>
        )}
      </div>
    </Card>
